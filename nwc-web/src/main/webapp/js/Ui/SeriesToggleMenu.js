...
NWCUI.ui.SeriesToggleMenuMixin = function () {
    ...
    var checkedSeriesButtons = {},
        monthlySeriesIdSet = {
            //Emulate a hash set using Javascript's builtin hash table
    ...
    self.toggleSeriesHandler = function (checkItem, checked, optionalGraph) {
        ...
        if (checkItem.chartColumn !== undefined || checkItem.seriesId) {
            if (checkItem.seriesId === 'deciles') {
                var i;
                for (i = lowestDecileGraphIndex; i < highestDecileGraphIndex; i++) {
        ...
        var isAMonthlySeriesId = function (id) {
            var isMonthlySeriesId = monthlySeriesIdSet.hasOwnProperty(id);
            return isMonthlySeriesId;
        };
