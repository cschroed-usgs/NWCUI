...
NWCUI.MapPanel = Ext.extend(GeoExt.MapPanel, {
    ...
    constructor: function (config) {
        ...
        var hucLayer = new OpenLayers.Layer.WMS(
...
//                                }
        ...
        bioDataGetFeatureControl.events.register('featuresselected', self, function (e) {
            ...
            var siteSelectionWin = new NWCUI.ui.BioDataSiteSelectionWindow({
                features: e.features
            });
    ...
    statsCallback: function (statsStores, success) {
        ...
        if (win.isVisible()) {//else, if the user has already closed the window, skip everything
            ...
            data = data.map(function (row) {
                ...
                var i;
                for (i = lowestDecileIndex; i < highestDecileIndex; i++) {
    ...
    wmsGetFeatureInfoHandler: function (responseObject) {
        ...
        if (hucLayerVisible) {
            ...
            if (hucFeatures.length) {
                ...
                var hucFields = [
                    {
                        name: self.fieldNames.huc12Id,
                        type: 'string'
                    },
                    {
                        name: self.fieldNames.huc12Name,
                        type: 'string'
                    },
                    {
                        name: self.fieldNames.huc12Area,
                        type: 'long'
                    }
                ];
                ...
                columnConfig[self.fieldNames.huc12Id] = {
                    header: 'HUC12'
                };
                columnConfig[self.fieldNames.huc12Name] = {
                    header: 'HUC12 Name'
                };
                columnConfig[self.fieldNames.huc12Area] = {
                    header: 'Area (Acres)'
                };
    ...
    updateFromClipValue: function (val) {
        ...
        var layerIdx;
        for (layerIdx = 0; layerIdx < this.map.layers.length; layerIdx++) {
